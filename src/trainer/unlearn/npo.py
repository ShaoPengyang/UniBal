...
def _to_model_inputs(batch):
    """Keep only model-forward keys and drop metadata like `index`."""
    return {
...
class NPO(GradDiff):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        forget_inputs = _to_model_inputs(inputs["forget"])
        ...
        retain_inputs = _to_model_inputs(inputs["retain"])
...
class DrNPO(NPO):
    ...
    def compute_dpo_loss(
        ...
        if win_inputs is not None:
            win_inputs = _to_model_inputs(win_inputs)
        ...
        if lose_inputs is not None:
            lose_inputs = _to_model_inputs(lose_inputs)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        retain_inputs = _to_model_inputs(inputs["retain"])
        retain_indices = inputs["retain"].get("index", None)

        assert (
            self.retain_loss_type == "NLL"
        ), "Only supports NLL retain loss currently."
        retain_loss, _ = compute_batch_nll(model, retain_inputs)
        valid_tokens = (retain_inputs["labels"][..., 1:] != -100).sum(dim=-1).clamp(min=1)
        per_sample_retain = retain_loss / valid_tokens.float()
        if hasattr(self, "update_retain_score_bank"):
            self.update_retain_score_bank(retain_indices=retain_indices, per_sample_losses=per_sample_retain)
        if self.retain_dro:
