...
# logger = logging.getLogger(__name__)  # 可选

# _FALLBACK_FORWARD_KEYS = {
#     "input_ids",
#     "attention_mask",
#     "labels",
#     "position_ids",
...
# def _filter_model_inputs_for_forward(model, inputs):
#     allowed_keys = None
#     try:
...
#         if has_var_kwargs:
#             # 包一层保守策略：即使有 **kwargs，也优先只放常见训练键，避免元数据继续下传
#             allowed_keys = set(_FALLBACK_FORWARD_KEYS)
#         else:
#             allowed_keys = {
#                 p.name
...
#     model_inputs = {k: v for k, v in inputs.items() if k in allowed_keys}

#     # NLL 计算依赖 labels，确保保留
#     if "labels" in inputs and "labels" not in model_inputs:
#         model_inputs["labels"] = inputs["labels"]

#     # 可选调试：观察被丢弃字段
#     # dropped = sorted(set(inputs.keys()) - set(model_inputs.keys()))
#     # if dropped:
#     #     logger.debug("Dropped non-forward keys in compute_batch_nll: %s", dropped)
...
def compute_batch_nll(model, inputs):
    ...
    # model_inputs = _filter_model_inputs_for_forward(model, inputs)
