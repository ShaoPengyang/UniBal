        ...
        for eps in "${eps_set[@]}"; do
            for eta in "${eta_set[@]}"; do
                for alpha in "${alpha_set[@]}"; do
                    for sampling_ratio in "${sampling_ratio_set[@]}"; do
                        # Args ========================================
                        forget_split=$(echo $split | cut -d' ' -f1)
                        holdout_split=$(echo $split | cut -d' ' -f2)
                        retain_split=$(echo $split | cut -d' ' -f3)

                        SUFFIX="lr${lr}_b${bsz}_ga${grad_acc}_eps${eps}_eta${eta}_a${alpha}_sr${sampling_ratio}_e${epochs}"
                        ...
                        python src/train.py --config-name=unlearn.yaml \
                            ...
                            trainer.method_args.sampling_ratio=$sampling_ratio \
